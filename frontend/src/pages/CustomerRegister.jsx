...
export default function CustomerRegister() {
  ...
  const handleAppleWallet = async () => {
    ...
    try {
      ...
      // Native navigation allows Safari to natively catch the application/vnd.apple.pkpass stream
      window.location.href = `/api/wallet/apple?data=${dataParam}`;

      // Reset loading state after a brief delay assuming successful handoff
      setTimeout(() => setAppleWalletLoading(false), 2000);
    } catch (err) {
      console.error('Apple Wallet error:', err);
      setWalletError('Error al generar Apple Wallet Pass.');
          ...
          <div style={s.walletBtns}>
            <button
              style={{ 
                ...
                background: '#000', 
                color: '#fff',
                opacity: appleWalletLoading ? 0.7 : 1,
              }}
